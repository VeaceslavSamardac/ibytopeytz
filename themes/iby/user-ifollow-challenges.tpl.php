...
<?php
  foreach($flags as $flag) {
		...
		if($flag->flag_group == 'challenge') {
			$loadChallenges = entity_load('taxonomy_term', array($flag->entity_id));
			
			foreach($loadChallenges as $forum) { ?>
	...
	<?php
			...
			")->fetchAll();
			//var_dump($getTopics);
			echo "<h3 style='margin-bottom:10px; font-size: 20px;'>Latest contributions</h3>";
